...
router.post('/del', function(req, res) {
    ...
    function readFile(i, files, folder, complete) {
        if (i < files.length) {
            fs.readFile('remove/' + username + '/' + files[i], function(err, data) {
                if (!err) {
    ...
    // 测试用户邮件文件夹是否存在
    fs.exists('remove/' + username,exists=>{
        if(exists){
            // 读取文件夹
    fs.readdir('remove/' + username, function(err, files) {
        var folder = []
